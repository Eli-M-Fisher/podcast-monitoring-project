...
def save_file_to_mongo(unique_id: str, file_path: str):
    ...
    try:
        ...
        logger.info(f"Connected to MongoDB at {MONGO_URI}, db={MONGO_DB}, collection={MONGO_COLLECTION}")
        ...
        with open(path, "rb") as f: # ("rb") to read binary files like .wav (l.f)
            ...
            logger.info(f"Stored file in MongoDB: filename={path.name}, unique_id={unique_id}, file_id={file_id}")
            return file_id
    
    except Exception as e:
        logger.error(f"Failed to save file {file_path} to mongodb (unique_id={unique_id}): {e}")
        raise